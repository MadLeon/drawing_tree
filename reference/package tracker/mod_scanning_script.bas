...
'/**
' * Detects whether a drawing sheet uses the new format (Job No column inserted at A/F)
' *
' * Old format: A=drawing, B+C=completion | E=drawing, F+G=completion
' * New format: A=Job No, B=drawing, C+D=completion | F=Job No, G=drawing, H+I=completion
' *
' * Detection scans rows 4-8 for RT- pattern in col A (old) vs col B (new).
' * Falls back to checking header rows for "Job" label when the sheet is empty.
' *
' * @param {Worksheet} ws - The drawing tracking worksheet to inspect
' * @return {Boolean} True if new format, False if old format
' */
Function DetectNewFormat(ws As Worksheet) As Boolean
    Dim r As Long
    Dim scanEnd As Long
    ...
    scanEnd = Application.Min(ws.Cells(ws.Rows.Count, "B").End(xlUp).Row, 8)
    If scanEnd < 4 Then scanEnd = 8

    For r = 4 To scanEnd
        colAVal = Trim(ws.Cells(r, 1).Value)
        colBVal = Trim(ws.Cells(r, 2).Value)
        If InStr(1, UCase(colAVal), "RT-") > 0 Then
            DetectNewFormat = False
            Exit Function
        End If
        If InStr(1, UCase(colBVal), "RT-") > 0 Then
            DetectNewFormat = True
            Exit Function
        End If
    Next r

    ' Empty sheet: check header rows for "Job" label in col A
    For h = 1 To 3
        If InStr(1, LCase(CStr(ws.Cells(h, 1).Value)), "job") > 0 Then
            DetectNewFormat = True
            Exit Function
        End If
    Next h
...
' *
' * Old format layout (auto-detected):
' * - Column A: drawing numbers | Columns B, C: completion dropdowns
' * - Column E: drawing numbers | Columns F, G: completion dropdowns
' *
' * New format layout (auto-detected, Job No columns A/F are ignored):
' * - Column B: drawing numbers | Columns C, D: completion dropdowns
' * - Column G: drawing numbers | Columns H, I: completion dropdowns
' *
' * Each page spans 44 rows starting at row 4.
...
Function FillExcelColumn(drawingData As Collection) As Boolean
    ...
    Set ws = ActiveSheet
    isNewFormat = DetectNewFormat(ws)
    ...
    Call LogInfo("Filling data in worksheet: " & ws.Name & " with hyperlinks and validations (format: " & IIf(isNewFormat, "new", "old") & ")", "FillExcelColumn")

    totalCount = drawingData.count
    rowsPerPage = 44
    dataIndex = 1
    pageIndex = 0        ' Page index starts at 0
    colIndex = 1         ' Column index: 1 for left side, 2 for right side
    ...
    While dataIndex <= totalCount
        ...
        If colIndex = 1 Then
            colLetter = IIf(isNewFormat, "B", "A")
        Else
            colLetter = IIf(isNewFormat, "G", "E")
        End If
            ...
            If dataIndex <= totalCount Then
                ...
                If colIndex = 1 Then
                    If isNewFormat Then
                        ...
                        Call AddValidationToCell(ws, poStatusSheet, "D" & currentRow)
                    Else
                ...
                Else
                    If isNewFormat Then
                        Call AddValidationToCell(ws, poStatusSheet, "H" & currentRow)
                        Call AddValidationToCell(ws, poStatusSheet, "I" & currentRow)
                    Else
