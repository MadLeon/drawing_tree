...
public class PoRepository
{
    ...
    /// <summary>
    /// Returns the set of part IDs (from the input list) that have at least one DIR attachment.
    /// </summary>
    public HashSet<int> GetPartIdsWithDir(IEnumerable<int> partIds)
    {
        var result = new HashSet<int>();
        var ids = partIds.ToList();
        if (ids.Count == 0) return result;
        try
        {
            ...
            using var cmd = conn.CreateCommand();
            var placeholders = string.Join(",", ids.Select((_, i) => $"@id{i}"));
            cmd.CommandText = $"SELECT DISTINCT part_id FROM part_attachment WHERE file_type = 'DIR' COLLATE NOCASE AND part_id IN ({placeholders})";
            for (int i = 0; i < ids.Count; i++)
                cmd.Parameters.AddWithValue($"@id{i}", ids[i]);
            using var reader = cmd.ExecuteReader();
            while (reader.Read())
        ...
        {
            Logger.Instance.Error($"PoRepository.GetPartIdsWithDir failed: {ex.Message}");
        }
        return result;
    }
